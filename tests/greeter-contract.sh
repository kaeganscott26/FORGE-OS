...
grep -Fq 'greetd-tuigreet-fork-git' "$root/scripts/configure-aur.sh" || fail 'AUR bootstrap does not provision the rolling maintained tuigreet fork.'

tuigreet_bin="${TUIGREET_BIN:-/usr/local/bin/tuigreet}"
[[ -x "$tuigreet_bin" ]] || tuigreet_bin="$(command -v tuigreet 2>/dev/null || true)"
[[ -n "$tuigreet_bin" && -x "$tuigreet_bin" ]] || fail 'maintained rolling tuigreet fork is not installed.'
...
for fork_option in --background --background-fps --kb-background --doom-height --doom-spread --doom-colors --matrix-length --matrix-speed --matrix-colors; do
  grep -Fq -- "$fork_option" <<<"$help_text" || fail "installed tuigreet does not contain the required rolling background API; missing $fork_option."
done
...
for config in "$root/config/greetd-config.toml" "$root/config/forge-recovery-greetd.toml" "$root/config/forge-live-greetd.toml"; do
  ...
  while IFS= read -r option; do
    grep -Fq -- "$option" <<<"$help_text" || fail "$config uses unsupported option $option for $("$tuigreet_bin" --version 2>&1 | head -n1)."
