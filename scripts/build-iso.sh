...
record_overlay_executable_permissions() {
  ...
  while IFS= read -r -d '' staged; do
    ...
    [[ "$relative" == /* && "$relative" != *'"'* && "$relative" != *$'\n'* ]] || {
      echo "Unsafe executable overlay path: $relative" >&2
      exit 1
    }
...
verify_squashfs_executables() {
  ...
  local -a critical=(
    ...
    usr/local/bin/forge-os-update
    usr/local/bin/forge-workspace-bootstrap
  ...
  for path in "${critical[@]}"; do
    ...
    expected='-rwxr-xr-x'
    [[ "$path" == */chrome-sandbox ]] && expected='-rwsr-xr-x'
    [[ "$mode" == "$expected" ]] || {
      echo "Built ISO executable mode mismatch for /$path: expected $expected, got ${mode:-missing}" >&2
      exit 1
    }
  done
}
