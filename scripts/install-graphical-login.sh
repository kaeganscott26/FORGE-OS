...
bash -n "$repository_root/session/forge-xsession"
...
sudo cmp -s "$repository_root/session/forge-xsession" /usr/local/bin/forge-xsession || {
  echo 'Installed graphical-session launcher does not match the repository.' >&2
  exit 1
}
